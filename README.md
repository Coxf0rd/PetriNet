...
Текущая версия: 0.7.171 (см. также `Cargo.toml`).
...
- Марковская модель:
  ...
  - режим дуг в рабочей области: `Aggregated` (агрегированные) или `All observed` (все наблюдённые);
  - два режима расчёта: точный (до 250_000 состояний) и примерный (по журналу симуляции при больших сетях);
