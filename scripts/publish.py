...
def update_cargo_lock(path: Path, new_version: str) -> None:
    ...
    # Cargo.lock может быть в CRLF, поэтому делаем правку построчно по блоку [[package]].
    ...
    while i < len(lines):
        ...
        while j < len(lines) and lines[j].strip() != "[[package]]":
            j += 1
        block = lines[i:j]
        if any(line.strip() == 'name = "petri_net_legacy_editor"' for line in block):
            for k in range(i, j):
                stripped = lines[k].strip()
                if stripped.startswith('version = "') and stripped.endswith('"'):
                    # Обновляем версию пакета проекта, даже если Cargo.lock был рассинхронизирован.
                    lines[k] = re.sub(r'\"[0-9]+\.[0-9]+\.[0-9]+\"', f"\"{new_version}\"", lines[k], count=1)
                    updated = True
                    break
            break
        i = j
