            ...
            .show(ctx, |ui| {
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        if ui
                            .button(self.tr(
                                "Включить марковскую модель для всех позиций",
                                "Enable Markov model for all places",
                            ))
                            .clicked()
                        {
                        ...
                        ui.label(self.tr(
                            "Марковская модель по позициям включается единой кнопкой выше",
                            "Markov model per place is enabled with the button above",
                        ));
                    });
                    ui.separator();
                    ui.add_space(6.0);
                    if let Some(chain) = &self.markov_model {
                        ...
                        if markov_focus_places.is_empty() {
                            ui.separator();
                            ui.label(self.tr(
                                "Нажмите кнопку выше, чтобы показать модель по позициям",
                                "Press the button above to show the model per place",
                            ));
                        } else {
                            ui.separator();
                            ui.label(
